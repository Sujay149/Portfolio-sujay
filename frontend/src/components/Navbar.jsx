    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-50/80 backdrop-blur-md">
      <div className="w-full px-6 lg:px-16 xl:px-32 py-6">
        <div className="flex items-center justify-between w-full">
          ...
          <Link to="/" className="absolute left-1/2 transform -translate-x-1/2 md:relative md:left-0 md:transform-none">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-black flex items-center justify-center text-white font-bold text-base md:text-lg hover:scale-110 transition-transform">
      ...
      <div className={`md:hidden fixed inset-0 top-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ease-in-out ${
        ...
        <div className="flex flex-col items-center justify-center h-screen gap-8 px-6" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="bg-white rounded-3xl border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8 flex flex-col gap-6 min-w-[280px]">
            <Link 
              to="/" 
              onClick={closeMobileMenu}
              className={`text-2xl font-bold transition-colors hover:text-black text-center ${
                isActive('/') ? 'text-black' : 'text-gray-700'
              }`}
            >
              Home
